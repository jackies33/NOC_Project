...
class Script(BaseScript, metaclass=MetricScriptBase):
    ...
    def set_metric(
        ...
        if sensor and sensor in self.seen_ids:
            ...
            if not labels:
                labels = id[1]
            #answer = f"TEST1{self.metric_labels}"
            #self.logger.info(answer)
            #lables_hash = self.get_labels_hash(*id)
            #answer = f"TEST2{lables_hash}"
            #self.logger.info(answer)
            ...
            if not mc:
                ...
                #answer = f"TEST4{id} = = = = Not requesting, ignoring"
    ...
    #### DOM metrics for tranciever telemetry on devices
    @metrics(
         [
    ...
    def collect_dom_metrics(self, metrics):
            for m in self.scripts.get_dom_status():
                 ...
                 if m.get("temp_c") is not None:
                     label_iface = f"noc::interface::{m['interface']}"#create lable of interface
                     self.set_metric(
                         id = ("Interface | DOM | Temperature", [label_iface]), #consider name of metrics noc system and label of interface
                         labels = [label_iface],
                         value = float(m['temp_c']),#consider the value of metric
                         multi = True, #True if single request can return several different labels.
                                         # When False - only first call with composite labels for same labels will be returned
                         units = "C"
                     )
                 if m.get("voltage_v") is not None:
                        ...
                        self.set_metric(
                                     id=("Interface | DOM | Voltage", [label_iface]),
                                     labels = [label_iface],
                 ...
                 if m.get("optical_rx_dbm") is not None:
                     label_iface = f"noc::interface::{m['interface']}"
                     self.set_metric(
                         id=("Interface | DOM | RxPower", [label_iface]),
                         labels=[label_iface],
                 ...
                 if m.get("current_ma") is not None:
                     label_iface = f"noc::interface::{m['interface']}"
                     self.set_metric(
                         id=("Interface | DOM | Bias Current", [label_iface]),
                         labels=[label_iface],
                         value=m['current_ma'],
                         multi=True,
                         units="A",
                 ...
                 if m.get("optical_errors_bip_ds") is not None:
                     label_iface = f"noc::interface::{m['interface']}"
                     #if isinstance(m['optical_errors_bip_ds'], int) and 0 <= m['optical_errors_bip_ds'] <= 4294967295:
                     self.set_metric(
                         id=("Interface | DOM | Errors | BIP | Downstream", [label_iface]),
                         labels=[label_iface],
                         value=m['optical_errors_bip_ds'],
                         multi=True,
                         units="1",
                         #scale= 0.000000001,
                     )
                 if m.get("optical_tx_dbm") is not None:
                     label_iface = f"noc::interface::{m['interface']}"
                     self.set_metric(
                         id=("Interface | DOM | TxPower", [label_iface]),
                         labels=[label_iface],
