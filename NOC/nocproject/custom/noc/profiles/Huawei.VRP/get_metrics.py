...
class Script(GetMetricsScript):
    ...
    def get_interface_load_persentage(self, metrics):
        """
        ...
        try:
            for index, n, s, l_i, l_o in self.snmp.join(
                    ["1.3.6.1.2.1.2.2.1.2", "1.3.6.1.2.1.2.2.1.5", "1.3.6.1.2.1.31.1.1.1.6",
                     "1.3.6.1.2.1.31.1.1.1.10"]):
                if l_i != 0:
                    first_poll_counters[n] = {
                        "l_i": l_i,
                        "l_o": l_o,
                    }
            ...
            for index, n, s, l_i, l_o in self.snmp.join(
                    ["1.3.6.1.2.1.2.2.1.2", "1.3.6.1.2.1.2.2.1.5", "1.3.6.1.2.1.31.1.1.1.6",
                     "1.3.6.1.2.1.31.1.1.1.10"]):
                if l_i != 0 and n in first_poll_counters:
                    l_i_diff = l_i - first_poll_counters[n]
                    l_i_diff = l_i_diff / s
                    lp_i = int(l_i_diff * 100)
                    label_iface = f"noc::interface::{str(n)}"
                    self.set_metric(
                        id=("Interface | Broadcast | In", [label_iface]),
                        labels=[label_iface],
                        value=lp_i,
                        multi=True,
                        units="pps",
                    )
                    l_o_diff = l_o - first_poll_counters[n]
                    l_o_diff = l_o_diff / s
                    lp_o = int(l_o_diff * 100)
                    label_iface = f"noc::interface::{str(n)}"
                    self.set_metric(
                        id=("Interface | Broadcast | Out", [label_iface]),
                        labels=[label_iface],
                        value=lp_o,
                        multi=True,
                        units="pps",
                    )
                #if l_o != 0 and n in first_poll_counters:

        except Exception as err:
            return f"\n\nError in get_metrics.py(Huawei.VRP)\n\n{err}\n\n"
    ...
    def get_interface_load_persentage(self, metrics):
        ...
        try:
            for index, n, s, l_i, l_o in self.snmp.join(["1.3.6.1.2.1.2.2.1.2", "1.3.6.1.2.1.2.2.1.5", "1.3.6.1.2.1.31.1.1.1.6" , "1.3.6.1.2.1.31.1.1.1.10"]):
                    if l_i != 0:
                        l_i = l_i * 8 / s
                        lp_i = int((l_i / s) * 100) # count value load of interface in %
                        label_iface = f"noc::interface::{str(n)}"
                        self.set_metric(
                            id=("Interface | Broadcast | In", [label_iface]),
                            labels=[label_iface],
                    ...
                    if l_o != 0:
                        l_i = l_i * 8 / s
                        lp_o = int((l_o / s) * 100)  # count value load of interface ще %
                        label_iface = f"noc::interface::{str(n)}"
                        self.set_metric(
                            id=("Interface | Broadcast | Out", [label_iface]),
                            labels=[label_iface],
