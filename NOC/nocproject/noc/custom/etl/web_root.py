...
import json
import sys
#from engine.id_range import change_var
from engine.put_command_noc import put_command_extract,put_command_check,put_command_load_add,put_command_load_update,put_command_wipe
...
def classifier_and_executor(event,time1,target,data_device_id,data_device_created,data_device_all,primary_ip):
    ...
    if event == "updated" and target == 'device':
        if dt_wh > dt_count:
            ...
            out = put_command_extract()
            if out == True:
                out1 = put_command_check()
                if out1 == True:
                    out_complete = put_command_load_update()
            ...
            if out == True:
                out1 = put_command_check()
                if out1 == True:
                    out_complete = put_command_load_add()
                    if out_complete == True:
    ...
    if event == "deleted" and target == 'device':
        ...
        out_delete = put_command_wipe(man_obj)
        if out_delete == True:
